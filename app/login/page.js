...
import styles from './login.module.css'; // Make sure this matches your CSS file name!
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setStatusMessage({ text: 'Verifying...', type: 'normal' });
    ...
    try {
      ...
      if (res.ok) {
        setStatusMessage({ text: 'Login successful! Welcome back.', type: 'success' });
  ...
  return (
    <div className={styles.container}>
      <div className={styles.formCard}>
        ...
        {/* The new link back to the Signup page */}
        <p style={{ marginTop: '1.5rem', textAlign: 'center', color: '#9ca3af', fontSize: '0.875rem' }}>
          Need an account?{' '}
          <Link href="/signup" style={{ color: '#3b82f6', textDecoration: 'none', fontWeight: 'bold' }}>
            Sign up
          </Link>
        </p>
      </div>
